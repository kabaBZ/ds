...
exchange = ccxt.binance(
    {
        ...
        "demo": True,
        "proxies": {
            "http": "http://127.0.0.1:2446",  # HTTP 代理地址
            "https": "http://127.0.0.1:2446",  # HTTPS 代理地址
        },
    }
)
            ...
            if pos["symbol"] == TRADE_CONFIG["symbol"]:
                ...
                if contracts > 0:
                    return {
                        ...
                        "size": contracts,
                        "entry_price": (
                            float(pos["entryPrice"]) if pos["entryPrice"] else 0
                        ),
                        "unrealized_pnl": (
...
def analyze_with_deepseek(price_data):
    ...
    for i, kline in enumerate(price_data["kline_data"][-10:]):
    ...
    position_text = (
        "无持仓"
        if not current_pos
        else f"{current_pos['side']}仓, 数量: {current_pos['size']}, 盈亏: {current_pos['unrealized_pnl']:.2f}USDT"
    )
    ...
    try:
        ...
        # 信号统计
        signal_count = len(
            [s for s in signal_history if s.get("signal") == signal_data["signal"]]
        )
        total_signals = len(signal_history)
        print(
            f"信号统计: {signal_data['signal']} (最近{total_signals}次中出现{signal_count}次)"
        )
    ...
    try:
        ...
        if required_margin > 0:
            if required_margin > usdt_balance * 0.8:
                print(
                    f"⚠️ 保证金不足，跳过交易。需要: {required_margin:.2f} USDT, 可用: {usdt_balance:.2f} USDT"
                )
                return
        else:
            print("✅ 无需额外保证金，继续执行")
